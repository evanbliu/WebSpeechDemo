document.addEventListener('DOMContentLoaded', () => {
    ...
    const qualitySelect = document.getElementById('quality-select'); // Added Quality Select
    ...
    function generateCode() {
        ...
        const quality = qualitySelect.value; // Get the selected quality
        ...
        // Create the string for quality to conditionally add to the template
        const qualityCode = quality ? `recognition.quality = "${quality}";` : '';
...
`;
    ...
    async function executeWithAudioTrack() {
        ...
        try {
            ...
            // Apply quality property if a valid option is selected
    ...
    async function checkAvailability() {
        ...
        try {
            ...
            // Create options and append quality if specified
            const options = {
                processLocally: true,
                langs: [lang]
            };
            ...
            // Format quality text for the output display if specified
            const qualityText = quality ? \` (Quality: \${quality})\` : '';
            outputEl.innerHTML = \`<p><strong>Availability for [\${lang}]\${qualityText}:</strong> \${availability}</p>\`;
    ...
    async function installModel() {
        ...
        try {
            ...
            const qualityText = quality ? \` (Quality: \${quality})\` : '';
            if (installed) {
                outputEl.innerHTML = \`<p><strong>Success!</strong> Model for [\${lang}]\${qualityText} has been installed.</p>\`;
            } else {
                outputEl.innerHTML = \`<p><strong>Installation Failed.</strong> The model for [\${lang}]\${qualityText} could not be installed. The browser may provide more details in the console.</p>\`;
            }
    ...
    qualitySelect.addEventListener('input', generateCode); // Added event listener for quality dropdown
